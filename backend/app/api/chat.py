...
def _relay_event_payload(row: AgentEvent) -> tuple[str, dict[str, object]]:
    """将数据库 AgentEvent 行转换为 SSE 中继的事件名和负载。

    Args:
        row: AgentEvent 数据库行。

    Returns:
        tuple[str, dict]: (事件名, 包含 sessionId/timestamp 等标准字段的负载字典)。
    """
    payload = dict(row.payload_json or {})
...
def _events_after_cursor(
    ...
    """查询指定游标之后的 AgentEvent 列表（排除 Span 事件）。

    使用 (created_at, id) 复合游标确保事件顺序正确，避免时间戳相同时的遗漏。

    Args:
        db: 数据库会话。
        tenant_id: 租户ID。
        session_id: 会话 ID。
        cursor: 上次读取位置 (created_at, id)，为 None 时从头开始。

    Returns:
        list[AgentEvent]: 游标之后的事件列表（最多 200 条）。
    """
...
def _sse(event: object, data: object, event_id: str | None = None) -> str:
    """格式化 SSE（Server-Sent Events）消息字符串。

    Args:
        event: 事件名称。
        data: 事件数据（将被 JSON 序列化）。
        event_id: 事件 ID（可选，用于 Last-Event-ID 重连续传）。
