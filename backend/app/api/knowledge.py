...
提供知识文档（KnowledgeDocument）的上传、解析、检索和相关资源管理功能：

- **文档上传**（``POST /documents``）：异步上传文档并创建知识摄取任务（IngestJob）。
- **OKF 导入**（``POST /okf/import``）：导入 OKF（Open Knowledge Format）概念包。
- **摄取任务**：查询任务状态、取消任务。
- **文档管理**：列表、详情、更新、分桶（bucket）和切片（chunk）查询。
- **知识检索**（``POST /search``）：基于向量+关键词的混合检索。
- **知识发现**（``/discoveries``）：LLM 自动从知识中发现技能草案，
  支持确认（confirm）和拒绝（reject）。

所有操作均遵循 agent 私有版本和开放广场（open gallery）的可见性规则。
"""
...
def _ensure_knowledge_version_visible(
    ...
    """验证知识库版本对当前 agent 范围可见。

    检查版本是否存在、属于正确的租户，以及关联的知识库
    在当前 agent 范围下是否可见。
    ...
    Raises:
        HTTPException: 版本不存在、租户不匹配或范围不可见时返回 404。
    """
