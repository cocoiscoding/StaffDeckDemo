"""知识库（Knowledge Base）管理 API 模块。

提供知识库的 CRUD、版本管理和 OKF 概念编辑功能：

- **CRUD**：创建、列表、详情、更新、删除知识库。
- **版本管理**：列出版本、查看版本详情。支持 agent 私有分支和主干版本的
  同步（``sync-from-overall``）、提升（``promote-to-overall``）和回滚（``rollback``）。
- **OKF 概念**：列出、查看、创建/更新 OKF 概念页，导出 OKF bundle，
  以及 lint 检查概念完整性。
- **分支隔离**：agent 范围下只能看到自己的分支版本；
  总管理员（overall agent）可管理开放广场中的所有知识库。
"""
...
def _management_knowledge_base_versions(
    ...
    agent 范围下返回该 agent 分支指向的版本；
    overall/无 agent 范围下返回开放广场中所有知识库的当前版本。
    ...
    Returns:
        ``{knowledge_base_id: KnowledgeBaseVersion}`` 映射字典。
    """
    agent = get_agent(db, tenant_id, agent_id)
    if agent and not agent.is_overall:
        branches = db.exec(
