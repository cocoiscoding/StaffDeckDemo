...
提供定时任务的 RESTful 接口，分为三组路由：

1. **企业端**（``/api/enterprise/scheduled-tasks``）：完整的 CRUD、执行记录查询、
   手动触发执行等管理操作。
2. **聊天端**（``/api/chat/scheduled-tasks``）：面向聊天界面的简化列表和创建接口，
   创建后会标记关联消息中的草案为已创建。
3. **聊天草案**（``/api/chat/scheduled-task-drafts``）：从用户消息中通过 LLM
   检测定时任务草案，返回可编辑的预填充表单。

所有接口均要求租户隔离和用户权限校验（管理员可查看所有用户的任务）。
"""
