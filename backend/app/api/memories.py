...
提供对长期记忆记录（MemoryRecord）的查询和清除功能。

- ``GET /api/enterprise/memories``：列出当前用户的记忆记录，支持按 agent、
  用户名、关键词过滤。管理员可查看所有用户的记忆。
- ``DELETE /api/enterprise/memories/me``：清除当前用户的全部非会话记忆。

记忆类型包括：``profile``（用户档案）、``preference``（偏好）、``fact``（事实）。
``conversation`` 类型的记忆不在此模块管理范围内。
"""
...
def _can_view_all_memories(
    ...
    Returns:
        True 如果可以查看所有用户的记忆，False 如果只能查看自己的。

    Raises:
        HTTPException: 如果 agent_id 指定但不存在，返回 404。
    """
