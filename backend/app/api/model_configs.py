"""模型配置（Model Config）API 模块。

提供 LLM 模型配置的管理和安全验证功能：

- **协议查询**（``GET /protocols``）：列出系统支持的模型 API 协议。
- **CRUD**：创建（``POST``）、更新（``PUT``）、列表（``GET``）模型配置。
- **设为默认**（``POST /{config_id}/set-default``）：将指定配置设为租户默认。
- **连接验证**（``POST /{config_id}/test``）：通过多轮探测（text/stream/json）
   验证模型连通性和能力，验证通过后自动更新信任状态。

安全模型：配置创建后默认 ``unverified`` 且禁用。修改 API key、base_url、
model 等安全相关字段后，信任状态会重置为 ``unverified`` 并自动禁用。
只有通过验证的配置才能被启用和设为默认。
"""
...
def _require_trusted(row: ModelConfig) -> None:
    ...
    只有通过连接测试的配置（``trust_status == "verified"`` 且指纹匹配）
    才能被启用或设为默认。``legacy_trusted`` 是向后兼容的特殊状态。

    Args:
        row: 模型配置行。

    Raises:
        HTTPException: 如果配置未通过验证，返回 409。
    """
...
def _fingerprint(row: ModelConfig) -> str:
    ...
    指纹覆盖 API 协议、base_url、model、key_revision、协议选项和 security_revision，
    用于检测安全相关字段是否被修改。
