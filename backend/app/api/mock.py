...
提供模拟的电商业务接口，用于开发、测试和演示场景。
所有接口需通过内部服务认证（``require_internal_service``）。

模拟数据包括：

- **商品目录**（``PRODUCT_CATALOG``、``PRODUCT_NAME_CATALOG``）：SKU 维度和商品名称维度的静态目录。
- **订单中心**（``PRIMARY_ORDER_CENTER``、``ARCHIVE_ORDER_CENTER``）：主订单中心和历史归档订单。
- **动态订单**：通过购买/创建操作写入数据库的 ``MockOrder`` 记录。

支持的操作：订单查询、归档查询、商品购买、商品价格查询、会员权益对账、
履约改道方案规划、订单创建等。
"""
