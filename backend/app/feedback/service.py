...
提供基于 LLM 的用户反馈（点赞/点踩）自动归因分析能力。

核心类 ``FeedbackAnalysisService`` 负责调用 LLM 对反馈进行分类，
将点踩反馈归因为以下桶（bucket）之一：

- ``model_issue``：模型理解/推理/回复问题
- ``skill_issue``：技能定义/步骤/槽位问题
- ``tool_or_system_issue``：工具调用/系统异常
- ``user_random_or_unclear``：用户随意或上下文不足
- ``positive_or_resolved``：正向反馈
- ``needs_model_analysis``：待模型分析
- ``unknown``：未知

还提供反馈汇总统计（``feedback_summary``）和读取格式化（``feedback_analysis_read``）等辅助函数。
"""
...
class FeedbackAnalysisService:
    """反馈分析服务，调用 LLM 对用户反馈进行自动归因。

    分析流程：收集反馈上下文（消息、会话状态、事件轨迹）→ 调用 LLM 分类 →
    归一化分析结果 → 持久化到 ``MessageFeedback`` 记录。
    支持 ``FEEDBACK_ANALYSIS_MAX_ATTEMPTS`` 次重试。
    ...
    def analyze_feedback(self, feedback_id: str) -> MessageFeedback | None:
        """分析指定反馈记录，调用 LLM 进行归因分类。

        流程：加载反馈 → 获取默认模型 → 收集上下文 → 调用 LLM →
        归一化结果 → 持久化。支持最多 ``FEEDBACK_ANALYSIS_MAX_ATTEMPTS`` 次重试。
        如果没有可用模型则标记为 ``needs_model_analysis``。

        Args:
            feedback_id: 反馈记录 ID。

        Returns:
            更新后的 ``MessageFeedback`` 实例，如果反馈不存在则返回 None。
        """
...
def _normalize_analysis(raw: dict[str, Any], rating: str) -> dict[str, Any]:
    """将 LLM 返回的原始分析结果归一化为标准格式。

    对 bucket 字段进行白名单校验和默认值兜底：
    如果是点赞且 bucket 为空/unknown/unclear，则修正为 ``positive_or_resolved``。
    不在白名单中的 bucket 统一设为 ``unknown``。

    Args:
        raw: LLM 返回的原始字典。
        rating: 反馈评分（``"up"`` 或 ``"down"``）。
