...
本模块为 ``tenant_demo`` 租户注入一套开箱即用的演示数据，使平台在首次启动后
即可体验完整的智能体对话能力，而无需手动配置。种子内容涵盖：

- 演示租户、用户（user_demo / admin）与系统人设提示词；
- 若干业务技能（售后退款/换货、购买商品、商品比价、图结构可视化验证），以
  nodes/edges 图模型定义，体现多分支、工具调用、知识检索等能力；
- 一组演示工具（订单查询、历史订单查询、购买、下单、价格查询）的 HTTP 定义；
- 两个 MCP 服务器（内置 builtin、stdio）及其预发现的子工具；
- 一个通用技能（中国城市天气，基于 Markdown + 附件）；
- 默认 LLM 模型配置（当提供 demo API key 时）。

入口函数 ``seed_demo_data`` 负责以幂等方式（已存在则跳过或合并更新）将上述
数据写入数据库，并通过 ``_publish_seeded_system_resources`` 把系统级资源发布到
开放画廊绑定。本模块与 ``database.py`` 的迁移逻辑互补：迁移负责结构演进，
本模块负责业务演示数据的初始化。
"""
...
# 自适应推进规则：注入到每个技能的 response_rules 中，约束模型不要把步骤当成
# 固定问答脚本，已满足的信息不得重复追问。
ADAPTIVE_FLOW_RULE = (
...
# --------------------------------------------------------------------------- #
# 演示技能定义（nodes/edges 图模型）。以下每个字典描述一个技能的内容（content_json）。
# --------------------------------------------------------------------------- #


# 售后退款技能：退款/退货/取消订单处理流程，含订单确认与资格查询。
REFUND_SKILL = {
...
# 购买商品技能：收集用户信息/商品/数量 → 确认 → 下单 → 反馈结果。
PURCHASE_SKILL = {
...
# 图结构可视化验证技能：显式 nodes + edges 定义，演示分支/可选/工具/知识/终止节点的展示效果。
...
# --------------------------------------------------------------------------- #
# 演示工具定义（HTTP 工具）。以下每个字典描述一个可被技能调用的 HTTP 工具配置。
# --------------------------------------------------------------------------- #


# 订单查询工具：按订单号查询状态、签收天数、是否可退款。
ORDER_QUERY_TOOL = {
    "name": "order.query",
    "display_name": "订单查询",
...
def _stdio_mcp_python() -> str:
    """返回用于启动 stdio MCP 子进程的 Python 解释器路径。

    打包态下 ``sys.executable`` 指向应用引导器而非真实 Python，因此需要改用
    应用附带的解释器；开发态直接使用当前解释器。

    Returns:
        str: 可用的 Python 解释器路径。
    """
...
def seed_demo_data(session: Session) -> None:
    """向 ``tenant_demo`` 租户注入全部演示数据（幂等）。

    按顺序创建/更新：租户、人设、用户（user_demo/admin）、默认智能体、
    各演示技能、演示工具、MCP 服务器与子工具、天气通用技能，发布系统资源，
    并在配置了 demo API key 时创建默认模型。已存在的记录会被跳过或合并更新，
    最后统一提交。
    ...
    if not demo_user:
        session.add(
            User(
                ...
                password_hash=settings.user_demo_password_hash or hash_password(settings.user_demo_password),
            )
        )
...
def _publish_seeded_system_resources(session: Session) -> None:
    """将种子产生的系统资源发布到整体智能体的开放画廊绑定。

    为整体智能体补充系统种子元数据、归档旧默认员工智能体，并把演示技能、演示
    工具、天气通用技能通过 ``ensure_open_gallery_binding`` 挂载到整体智能体，
    使其在前端画廊中可见。
...
def _archive_seed_default_agent(session: Session, tenant_id: str) -> None:
    ...
    仅当目标智能体确属系统/管理员创建时，才将其状态置为 archived、补充隐藏与
    归档标记的元数据；否则保留不动。
...
def _system_seed_metadata(extra: dict[str, object] | None = None) -> dict[str, object]:
    ...
    将一组 admin 归属字段（owner/created_by/creator_name 等）合并进传入的
    额外元数据，用于标记由系统种子创建的资源。

    Args:
        extra: 需要保留并合并的额外元数据（可选）。

    Returns:
        dict[str, object]: 合并后的元数据字典。
    """
...
def _tool_config_with_base_url(tool_config: dict, base_url: str) -> dict:
    """返回工具配置的副本，并将其 url 前置工具基础地址。

    Args:
        tool_config: 原始工具配置字典。
        base_url: 工具服务基础地址。

    Returns:
        dict: url 已补全基础地址的工具配置副本。
    """
...
def _tool_url_with_base(url: str, base_url: str) -> str:
    """将相对路径的工具 url 拼接上基础地址；绝对地址原样返回。

    Args:
        url: 工具原始 url（可能是 ``/api/...`` 相对路径或完整 url）。
        base_url: 工具服务基础地址。
...
def _seed_weather_general_skill(session: Session) -> None:
    """从本地文件加载“中国城市天气”通用技能并写入演示租户（若源文件存在）。

    尝试从开发者本地约定的路径读取天气技能的 Markdown 与附件包；若文件不存在
    则跳过。已存在记录时按需更新正文/状态/附件，缺失时新建为 published 状态。
...
def _collect_general_skill_folder(folder: Path) -> list[dict[str, object]]:
    """收集通用技能文件夹下的全部文本文件，组装为 skill_files_json 结构。

    递归扫描 ``folder`` 下所有非隐藏文件，读取文本内容，记录相对路径、大小与
    MIME 类型；非文本文件（解码失败）会被跳过。要求文件夹下存在 ``SKILL.md``。

    Args:
        folder: 通用技能源文件夹路径。

    Returns:
        list[dict[str, object]]: 文件描述列表；文件夹无效或无 SKILL.md 时返回空列表。
    """
...
def _sync_demo_skill_if_stale(existing: Skill, desired: dict) -> None:
    """将期望的技能内容合并进已存在的演示技能（仅在内容过时时更新）。

    以期望的节点定义为基准，按 node_id 合并节点字段、补齐缺失 instruction、
    同步 edges/start_node_id/terminal_node_ids/required_info/interruption_policy，
    并合并 slot_filling_policy（target_info 取并集）与 response_rules（追加缺失项）。
    任何字段发生变更才会更新 ``content_json`` 与时间戳，避免无谓写入。

    Args:
        existing: 数据库中已存在的技能对象。
        desired: 期望的最新技能内容字典。
    """
...
def _skill_content_graph(content: dict) -> dict:
    """为技能内容补全图谱元数据（start_node_id/terminal_node_ids/edges）。

    若内容已含 nodes，则按节点顺序推导默认的起始/终止节点与线性 edges（仅在
    缺失时通过 ``setdefault`` 补充）；若无 nodes，则补建空图谱骨架。

    Args:
        content: 技能内容字典（不会被强制修改既有字段）。
...
def _merge_slot_filling_policy(current: object, desired: dict) -> dict:
    """合并槽位填充策略：以 desired 覆盖 current，target_info 取并集后排序。

    Args:
        current: 现有策略（dict 或其他类型）。
        desired: 期望策略。
...
def _append_missing_rules(current: object, desired: list[str]) -> list[str]:
    """将缺失的响应规则追加到现有规则列表末尾（保持顺序、去重）。

    Args:
        current: 现有规则列表（list 或其他类型）。
        desired: 期望补充的规则列表。
