/**
 * @file StaffdeckIcon.tsx
 * @module components/StaffdeckIcon
 * @description StaffDeck 内置 SVG 图标组件。维护一套以路径数据（path d）驱动的
 *              图标字典，按名称渲染对应的 24×24 视口 SVG 图标，供全局复用。
 */

import type { CSSProperties } from 'react';

/**
 * 支持的图标名称枚举。每个值对应 iconPaths 中的一组 SVG 路径数据。
 */
...
type StaffdeckIconProps = {
  /** 图标名称，必须为 StaffdeckIconName 中的有效值。 */
  name: StaffdeckIconName;
  /** 附加到根 SVG 元素上的 CSS 类名。 */
  className?: string;
  /** 图标宽高（px），默认 18。 */
  size?: number;
  /** 行内样式覆盖。 */
...
/**
 * 图标路径字典：键为图标名称，值为 SVG <path> 的 d 属性数组。
 * 一个图标可由多条路径组合而成。
 */
...
/**
 * StaffDeck 内置 SVG 图标组件。按名称从路径字典中取出对应的 path 数据，
 * 渲染为统一 24×24 视口的 <svg> 元素。
 *
 * @param props - 图标属性
 * @param props.name - 图标名称
 * @param props.className - 附加 CSS 类名
 * @param props.size - 图标尺寸（px），默认 18
 * @param props.style - 行内样式
 * @returns 渲染好的 SVG 图标元素
 */
