 ...
 * @description 应用侧边栏组件。支持两种变体：
 *              1. management（管理端）：展示主导航、员工切换器、基本资料/员工能力子菜单、对话端入口。
 *              2. chat（对话端）：展示数字员工广场入口、会话筛选器、会话列表、待回答入口、管理端入口。
 *              两种变体均支持展开与折叠（icon-only）两种显示状态，共享相同的外壳样式与品牌头部。
 */
...
/**
 * 侧边栏底部操作区组件（管理端展开态）。
 * 包含"对话端"入口按钮和切换到对话端的开关按钮。
 * @param onOpenChat - 打开对话端回调
 * @returns 底部操作区 JSX
 */
...
/**
 * 管理端侧边栏组件（展开/折叠自适应）。
 * 根据侧边栏折叠状态在折叠态和展开态之间切换，
 * 展开时展示品牌头部、主导航、员工切换器、基本资料/员工能力子菜单和对话端入口。
 * @param props - 管理端侧边栏属性
 * @returns 管理端侧边栏 JSX
 */
...
/**
 * 根据会话的 agent_id 从完整员工列表中查找对应的员工数据。
 *
 * @param session - 聊天会话
 * @param agents - 完整员工列表
 * @returns 匹配的员工数据，未找到时返回 null
 */
...
/**
 * 获取会话标题：优先使用会话自身标题，否则回退到会话 ID 或"新对话"。
 *
 * @param session - 聊天会话
 * @param _agent - 关联员工（当前未使用）
 * @returns 会话显示标题
 */
...
/**
 * 获取会话副标题：取最近一条员工回复或摘要，去除"最近回复:"前缀。
 *
 * @param session - 聊天会话
 * @param _agent - 关联员工（当前未使用）
 * @returns 会话副标题文本
...
/**
 * 会话列表骨架屏组件。
 * 渲染多行骨架屏占位，用于会话列表加载中的视觉反馈。
 * @param rows - 骨架屏行数，默认 5
 * @returns 骨架屏列表 JSX
 */
 ...
 * 对话端侧边栏组件（展开/折叠自适应）。
 * 根据侧边栏折叠状态在折叠态和展开态之间切换，
 * 展开时展示品牌头部、数字员工广场入口、待回答入口、
 * 会话筛选器、会话列表和管理端入口。
 * @param props - 对话端侧边栏属性
 * @returns 对话端侧边栏 JSX
 */
...
/**
 * 应用侧边栏主入口组件。根据 variant 属性分发到管理端或对话端侧边栏。
 *
 * @param props - 侧边栏属性（管理端或对话端联合类型）
 * @returns 对应变体的侧边栏组件
 */
