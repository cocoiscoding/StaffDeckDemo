 ...
 * @module pages/KnowledgePage
 * @description 知识库管理页面。展示知识库和文档列表、文档解析进度、搜索筛选，
 *              支持创建知识库、上传/解析文档、查看/编辑文档内容、启用/停用/删除文档，
 *              以及从广场或数字员工复制知识库等操作。
 */
...
/**
 * 返回有效的非全局员工作用域 ID。
 * 若作用域为全局员工则返回空字符串（用于全局广场模式）。
 * @param rows - 全部员工列表
 * @param agentId - 当前作用域 ID
 * @returns 非全局员工 ID 或空字符串
 */
...
export default function KnowledgeManagePage({ currentUser, onLogout }: KnowledgePageProps = {}) {
  ...
  /**
   * 刷新知识库和文档列表。
   * 按当前作用域并行加载文档和知识库，并自动选中第一个文档或加载知识图谱概念。
   * @param scopedAgentId - 指定的员工作用域 ID，默认为当前有效作用域
   */
  ...
  /**
   * 打开知识库导入弹窗。
   * 根据模式（广场/员工）加载来源员工列表，并预加载该来源的知识库。
   * @param mode - 导入模式：'plaza' 从广场导入 / 'employee' 从其他员工导入
   * @param selectedResourceId - 可选的预设资源 ID，用于从 URL 参数自动选中
   */
  ...
  /**
   * 处理创建知识库的操作分流。
   * @param key - 创建类型：'blank' 创建空白知识库 / 'okf' 导入 OKF 文件 / 'import' 从广场/员工导入
   */
   ...
   * 导出指定知识库的 OKF 备份包。
   * 下载为 ZIP 文件并触发浏览器保存。
   * @param targetKnowledgeBase - 目标知识库，默认为当前选中的知识库
   */
  ...
  /**
   * 渲染知识库列表行的操作菜单。
   * 根据权限和知识库状态展示编辑、上线/下线、同步、发布、导出、删除等操作。
   * @param item - 知识库数据
   * @returns 操作菜单 JSX
   */
...
export function KnowledgeAddPage({ currentUser }: KnowledgePageProps = {}) {
  ...
  /**
   * 加载指定入库任务关联的知识发现建议。
   * 入库成功后检查是否有待确认的 SOP/工具发现建议，有则弹出确认弹窗。
   * @param job - 已完成的入库任务
   */
...
/**
 * 知识检索调试结果展示组件。
 * 以可折叠面板形式展示检索路由轨迹、命中的知识图谱概念和引用来源。
 * @param result - 检索结果
 * @param loading - 是否正在检索
 * @param compact - 是否使用紧凑布局
 * @returns 检索调试展示 JSX
 */
...
/**
 * 根据状态值返回对应的彩色状态标签。
 * @param status - 状态值（如 active、pending、failed 等）
 * @returns 状态标签组件
 */
...
/**
 * 知识库页面通用弹窗组件。
 * 基于基础 Dialog 封装，统一标题、内容区和底部按钮布局。
 * @param open - 是否打开
 * @param title - 弹窗标题
 * @param width - 弹窗最大宽度（数字为 px）
 * @param className - 内容区自定义类名
 * @param footer - 底部按钮区
 * @param onClose - 关闭回调
 * @param children - 弹窗内容
 * @returns 弹窗 JSX
...
/**
 * 计算知识图谱概念的分组键。
 * 优先使用来源文档，无来源时按概念类型分组。
 * @param concept - 概念条目
 * @returns 分组键（"source:文档名" 或 "type:类型"）
 */
 ...
 * @param markdown - 含 frontmatter 的 Markdown 文本
 * @param key - 要提取的键名
