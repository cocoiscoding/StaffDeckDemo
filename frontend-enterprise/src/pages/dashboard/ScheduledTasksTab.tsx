 ...
 * @file 定时任务标签页组件
 * @description
 * 仪表盘"定时任务"标签页，管理当前数字员工的定时任务及其执行记录。
 *
 * 核心功能：
 * - 展示定时任务列表（标题、计划、状态、下次执行、已执行次数、最近结果）
 * - 展示执行记录列表（任务名、状态、计划/完成时间、结果摘要、会话链接）
 * - 支持新增、编辑、启用/暂停、立即执行、删除定时任务
 * - 支持查看单个任务的执行记录弹窗
 * - 按任务状态和执行结果进行筛选
 *
 * 数据按员工范围隔离，必须先选中具体员工（非全局概览）才能配置定时任务。
 */
