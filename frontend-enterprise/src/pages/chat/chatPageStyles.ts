/**
 * @file chatPageStyles.ts
 * @module pages/chat/chatPageStyles
 * @description
 * 聊天页面的 Tailwind CSS 样式常量集合。
 *
 * 本文件将聊天界面的所有 Tailwind class token 集中管理，确保视觉一致性
 * 并便于全局调整。样式基于 SD1 Figma 设计稿（节点 38:3773 空状态、
 * 38:4962 活跃对话），采用以下设计规范：
 *
 * - 背景色：#fcfcfc（主区域）、#f6f6f6（用户气泡/卡片）
 * - 侧边栏：220px 宽，右边框 #f4f4f4
 * - 主色：ink #18181a（深色文字/按钮）
 * - 次要色：muted #858b9c / #757f9c（灰色文字）
 * - 边框：hairline #e3e7f1（0.5px 细线）
 * - 标题栏：56px 高
 * - 输入框：白色卡片，#18181a 边框，14px 圆角，深色发送按钮
 *
 * 样式按功能分区：共享图标、标题栏、消息列表、气泡、Markdown、
 * 执行记录、引用、附件、反馈、空状态、输入框、定时任务草案、弹窗。
 */

import { cn } from '@/lib/utils';

/**
 * Tailwind class token 集合，用于迁移后的聊天对话页面。
 *
 * 色彩与尺寸参数来源于 SD1 Figma 设计稿。
 * 聊天侧边栏本身现在位于可复用的 AppSidebar 组件中
 * （参见 AppSidebar.tsx `variant="chat"`）。
 */
...
/**
 * 根据消息角色生成消息行的 CSS class。
 * 用户消息右对齐，其他角色左对齐。
 *
 * @param role - 消息角色
 * @returns 组合后的 CSS class 字符串
 */
...
/**
 * 根据消息角色和错误状态生成消息气泡的 CSS class。
 * 用户消息使用灰色背景气泡，AI/系统消息使用白色边框卡片气泡。
 * 错误消息使用红色背景和边框。
 *
 * @param role - 消息角色
