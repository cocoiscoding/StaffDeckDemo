 ...
 * @module pages/chat/useChatSession
 * @description
 * 聊天会话核心 Hook（约 3373 行），是整个前端应用中最复杂的单个文件。
 *
 * 本 Hook 封装了聊天界面的全部状态管理和业务逻辑，作为单一数据源
 * 向所有聊天页面组件提供状态和操作方法。
 *
 * ## 核心职责
 *
 ...
 * - 加载和维护会话列表（sessions）
 * - 会话切换、创建草稿会话、重命名、删除
 * - 会话已读状态追踪（sessionReadTimes）
 * - 侧边栏折叠状态
 *
 * ### 2. 消息生命周期
 * - 从服务端加载会话消息（serverMessages）
 * - 管理实时流式消息（realtimeMessages）
 * - 消息合并与去重（computeMergedMessages）
 * - 发送新消息（send）和中止流式回复（abortStream）
 *
 * ### 3. 流式响应处理
 * - 通过 SSE 接收流式事件（streamChatTurn）
 * - 流式文本增量更新（stream_replace / stream_delta / token）
 * - 流式心跳检测和空闲超时处理
 * - 中继恢复（relay recovery）机制
 *
 * ### 4. 思维链追踪（Trace）
 * - 实时解析流式事件为结构化追踪行
 * - 支持技能、工具、代码、知识检索等多种追踪类型
 * - 追踪行的合并、折叠/展开管理
 *
 * ### 5. 消息队列
 * - 当 AI 正在回复时，新消息加入队列等待发送
 * - 队列持久化到 sessionStorage，防止刷新丢失
 * - 自动出队机制（drainQueuedTurns）
 *
 * ### 6. 定时任务
 * - 定时任务草案的创建、编辑、确认
 * - 已创建任务的状态追踪
 ...
 * ## 架构模式
 *
 * - 使用 useRef 存储不需要触发重渲染的可变状态（storeRef、streamRef 等）
 * - 使用 useState 存储 UI 需要响应的 tick 计数器（storeTick、streamTick 等）
 * - 通过 notify* 回调函数触发对应的 tick 更新，间接驱动重渲染
 * - 大量使用 useCallback 缓存回调函数，避免子组件不必要的重渲染
 *
 * @example
 * ```tsx
 * function ChatPage() {
...
// 企业侧边栏折叠状态的 localStorage 键，与管理端共享（见 App.tsx）。
// 存储为 '1'（展开）/ '0'（折叠）；未设置时默认展开。
const ENTERPRISE_SIDEBAR_STORAGE_KEY = 'ultrarag_enterprise_sidebar_expanded';
/** 用于匹配"模型配置缺失"错误的正则表达式（支持中英文） */
 ...
 *
 * @param id - 会话 ID
 * @returns 格式为 /workspace/chat/{id} 的路由路径
 */
...
/**
 * 将排队轮次转换为用于 UI 预览的聊天消息对象。
 * 生成一个带有 queued: true 标记的用户消息，用于在界面上显示排队中的消息。
 *
 * @param turn - 排队的聊天轮次
 * @returns 带有排队标记的 ChatMessage 预览对象
 */
...
/**
 * useChatSession Hook 的返回值类型。
 * 所有聊天组件通过此类型访问聊天状态和操作方法。
 */
export type UseChatSession = ReturnType<typeof useChatSession>;

/**
 * useChatSession Hook 的配置选项。
 */
export type UseChatSessionOptions = {
  /**
   * 匿名模式（用于公开站点嵌入）：
   * - true：遇到认证失败时不跳转登录页，认证相关数据保持为空
   * - false（默认）：遇到认证失败时通过 window.location.href = '/' 跳转登录
   */
  anonymous?: boolean;
};
 ...
 * 聊天会话核心 Hook。
 *
 * 这是整个聊天模块的状态管理中心，封装了会话管理、消息收发、流式响应处理、
 * 思维链追踪、消息队列、定时任务、人工接续等全部业务逻辑。
 *
 * 使用方式：
 * ```tsx
 * const chat = useChatSession();
 * // chat.displayedMessages - 当前显示的消息列表
 * // chat.send() - 发送消息
 * // chat.input / chat.setInput - 输入框文本
 * ```
 *
 * @param options - 配置选项
 * @returns 包含全部聊天状态和操作方法的对象
 */
export function useChatSession(options: UseChatSessionOptions = {}) {
  ...
  // ===========================================================================
  // 状态声明区
  // 采用 useRef + useState tick 的模式管理复杂可变状态：
  // - useRef 存储 Map/Set 等可变数据（不触发重渲染）
  // - useState tick 计数器用于在需要时手动触发重渲染
  // - notify* 回调递增对应的 tick，间接驱动 UI 更新
  // ===========================================================================
  ...
  // Refs 区（不触发重渲染的可变引用）
  // ===========================================================================
  const chatMessagesRef = useRef<HTMLDivElement>(null);          // 消息列表滚动容器
  const fileInputRef = useRef<HTMLInputElement>(null);           // 隐藏的文件选择输入
  const isChatProgrammaticScrollRef = useRef(false);             // 标记是否为程序化滚动（避免与用户滚动冲突）
  const isChatStickyToBottomRef = useRef(true);                  // 消息列表是否粘在底部（用于自动滚动控制）
  const lastActiveConversationIdRef = useRef<string | null>(null); // 上一次活跃的会话 ID
  const lastDisplayedMessageCountRef = useRef(0);                // 上一次显示的消息数量（用于检测新消息）
  const terminalTurnSyncRef = useRef(new Map<string, { startedAt: number; timer: number | null }>());
  const storeRef = useRef(new Map<string, SessionSlot>());       // 各会话的消息槽位（服务端 + 实时消息）
  const streamRef = useRef(new Map<string, StreamSlot>());       // 各会话的流式响应状态
  const turnTraceRef = useRef(new Map<string, TurnTrace>());     // 各轮次的思维链追踪数据
  const locallyCancelledSessionIdsRef = useRef(new Set<string>()); // 本地取消的会话 ID 集合
  const scheduledEventIdsRef = useRef(new Set<string>());        // 已处理的定时事件 ID
  const knownSessionIdsRef = useRef(new Set<string>());          // 已知的会话 ID 集合
  const optimisticSessionIdsRef = useRef(new Set<string>());     // 乐观创建的会话 ID（尚未确认持久化）
  const pendingPromotedSessionIdRef = useRef<string | null>(null);
  const queuedTurnsRef = useRef<PreparedChatTurn[]>(restoredQueuedTurns); // 待发送的消息队列
  const queuedTurnProcessingRef = useRef(false);                 // 是否正在处理队列中的消息
  const queuedTurnPreviewsRestoredRef = useRef(false);           // 队列预览是否已恢复
  const sessionsInitializedRef = useRef(false);                  // 会话列表是否已初始化
  const autoOpenedSessionIdsRef = useRef(new Set<string>());     // 自动打开过的会话 ID
  const loadErrorNoticeRef = useRef<Record<string, number>>({}); // 加载错误通知的时间记录
  const uploadControllersRef = useRef(new Map<string, AbortController>()); // 附件上传的中止控制器

  // --- notify 回调：通过递增 tick 计数器触发对应模块的重渲染 ---
  const notifyStore = useCallback(() => setStoreTick((value) => value + 1), []);       // 通知消息存储变更
  const notifyStream = useCallback(() => setStreamTick((value) => value + 1), []);     // 通知流式状态变更
  const notifyTrace = useCallback(() => setTraceTick((value) => value + 1), []);       // 通知追踪数据变更
  const notifyFeedback = useCallback(() => setFeedbackTick((value) => value + 1), []); // 通知反馈状态变更
  const notifyQueue = useCallback(() => setQueuedTurnsTick((value) => value + 1), []); // 通知队列变更
