/**
 * @file ModelSetupDialog.tsx
 * @module pages/chat/components/ModelSetupDialog
 * @description
 * 模型配置向导弹窗组件。
 *
 * 当系统检测到没有可用的 AI 模型配置时，弹出此向导引导用户完成配置：
 * - 填写配置名称、API 协议（OpenAI / Anthropic / Gemini）
 * - 填写 Base URL、模型标识、API Key
 * - 设置 Temperature 和 Max Tokens
 * - 保存后自动执行连通性测试
 * - 测试通过后激活为默认模型
 *
 * 如果用户没有模型管理权限，则显示提示信息引导联系管理员。
 */
...
export default function ModelSetupDialog({
  ...
  /**
   * 保存模型配置并执行连通性测试。
   * 流程：验证表单 → 保存/更新配置 → 执行测试 → 激活默认模型。
   * @throws 测试失败时设置错误结果（不抛出异常）
   */
 ...
 *
 * @param label - 字段标签文本
 * @param children - 字段内容（输入控件）
 * @returns 带标签的表单字段 JSX
 */
