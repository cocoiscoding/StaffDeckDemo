/**
 * @file Composer.tsx
 * @module pages/chat/components/Composer
 * @description
 * 聊天输入框（编辑器）组件。
 *
 * 这是用户发送消息的核心交互组件，功能包括：
 * - 多行文本输入（自动高度调整，最大 200px）
 * - 文件上传（点击"+"按钮选择文件，或拖放文件，或粘贴图片）
 * - 附件预览（显示文件名、类型、上传状态，可移除）
 * - 交互模式切换（普通对话 / 定时任务）
 * - 模型选择下拉菜单（切换当前使用的 AI 模型）
 * - 模型配置提示（无可用模型时显示配置按钮）
 * - 发送/停止按钮（AI 回复中显示停止按钮，发送的新消息进入队列）
 * - 员工头像悬浮卡片（显示当前数字员工信息）
 * - 键盘快捷键（Enter 发送，Shift+Enter 换行，支持中文输入法合成检测）
 */
