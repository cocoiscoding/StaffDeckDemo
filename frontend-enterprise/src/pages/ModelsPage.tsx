/**
 * @file pages/ModelsPage.tsx
 * @description 模型配置管理页面。
 *
 * 管理企业端使用的大语言模型配置，包括：
 * - 模型列表（支持搜索筛选、前端分页）
 * - 新建/编辑模型配置（名称、协议、Base URL、模型 ID、密钥、温度等）
 * - 设为默认模型、启用/禁用
 * - 连接测试（验证模型配置是否可用）
 * - 统计卡片（模型总数、默认模型、已启用数等）
 *
 * 数据通过 /api/enterprise/model-configs 接口交互。
 */
...
export default function ModelsPage({
  ...
  /**
   * 从后端加载全部模型配置。
   * 加载完成后广播 MODEL_CONFIGS_UPDATED_EVENT 事件通知其他组件。
   * @param showLoading 是否显示加载态
   */
  ...
  /**
   * 保存模型配置：校验名称、模型 ID、数值字段和 JSON 格式后，
   * 根据是否选中已有模型决定 POST（新建）或 PUT（更新）。
   */
  async function save() {
  ...
  /**
   * 测试模型连接：向后端发送测试请求验证模型配置是否可用。
   * 使用 ref + state 双重管理测试中的模型 ID 集合，防止重复测试。
   * 首次测试通过时会自动启用并设为默认模型。
   * @param row 待测试的模型配置
   */
