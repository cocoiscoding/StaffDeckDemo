/**
 * @file pages/DistillPage.tsx
 * @description 技能（SOP）蒸馏/编辑器页面 — StaffDeck 最复杂的核心页面。
 *
 * 该页面是一个 AI 辅助的 SOP 技能编辑器，通过对话式交互引导用户
 * 从自然语言描述中提炼出结构化的技能流程图（Skill Flow）。
 *
 * 核心能力：
 * - 对话式蒸馏：用户用自然语言描述需求，AI 自动生成/修改 SOP 节点、
 *   槽位、动作白名单、条件分支和回复规则
 * - 可视化流程图：以 DAG 图形化展示 SOP 的节点与边（支持前向/回退/条件分支）
 * - 源码视图：以结构化 JSON 源码形式查看和编辑技能定义
 * - 实时差异高亮：AI 修改时高亮显示新增/删除/变更的字段
 * - 版本管理：保存时生成版本号，支持历史回滚
 * - 工具探测：可测试技能中引用的工具调用
 * - 附件上传：支持在对话中上传文件作为上下文
 * - 草稿缓存：支持将未保存的编辑缓存在 localStorage
 *
 * 页面通过 URL 参数控制模式：
 * - mode=create：新建技能
 * - skill_id=xxx：编辑已有技能
 * - agent_id=xxx：绑定到指定数字员工
 */
...
/** 蒸馏历史快照：在 AI 回复前后保存完整页面状态，用于回滚/重跑 */
...
/**
 * 将技能草稿的 skill_id 锁定为指定值（深拷贝后替换）。
 * 用于在多员工场景下确保草稿归属正确的技能 ID。
 * @param draft 原始技能草稿
 * @param lockedSkillId 要锁定的技能 ID
 * @returns 锁定后的技能草稿
 */
...
/**
 * lockSkillIdForDraft 的空安全版本，处理 null 草稿。
 * @param draft 可能为 null 的技能草稿
 * @param lockedSkillId 要锁定的技能 ID
 * @returns 锁定后的草稿或 null
 */
...
/**
 * 锁定待确认变更（PendingChange）中前后两个草稿的 skill_id。
 * @param change 待确认变更对象
 * @param lockedSkillId 要锁定的技能 ID
 * @returns 锁定后的变更对象
 */
 ...
 *
 * @param props - 组件属性
 * @param props.active - 页面是否处于活跃状态
 * @param props.searchParamsOverride - 搜索参数覆盖（用于嵌入模式）
 * @param props.currentUser - 当前登录用户
 * @param props.onLogout - 退出登录回调
 * @returns 渲染好的 SOP 提炼页面
 */
...
/**
 * 蒸馏页面的分区卡片容器组件。
 * 提供统一的卡片外观（边框、圆角、阴影），包含可选的标题栏和内容区。
 * @param props.className 外层自定义样式
 * @param props.bodyClassName 内容区自定义样式
 * @param props.title 卡片标题（可选）
 * @param props.extra 标题栏右侧附加内容（可选）
 * @param props.children 卡片内容
 * @returns 渲染好的 section 卡片
 */
...
/**
 * SOP 源码视图组件：以结构化表单形式展示和编辑技能的完整定义。
 * 包括技能元信息、节点列表、边列表和工具绑定。
 * 支持字段级别的选中高亮、更新动画和差异对比。
 * @param props.skill 技能草稿
 * @param props.selectedPaths 当前选中的字段路径列表
 * @param props.highlightedPaths 高亮显示的字段路径列表
 * @param props.updatingPaths 正在更新动画中的字段路径列表
 * @param props.dirtyPaths 有未保存修改的字段路径列表
 * @param props.textDiffs 文本差异动画列表
 * @returns 渲染好的源码编辑视图
 */
...
/**
 * SOP 流程图视图组件：以可视化 DAG 图形展示技能的节点和边。
 * 基于 buildSkillFlowCanvasLayout 计算的布局坐标，渲染 SVG 画布上的
 * 节点卡片和连接路径。支持前向边、条件分支、回退边和并行边的
 * 不同视觉样式（颜色、标签）。
 * @param props.skill 技能草稿
 * @param props.selectedPaths 选中的字段路径（用于高亮对应节点/边）
 * @param props.highlightedPaths 高亮的字段路径
 * @param props.containerRef 画布容器引用
 * @returns 渲染好的流程图画布
 */
function SkillFlow({
  skill,
...
/**
 * 将技能草稿的节点列表转换为标准的流程图步骤数组。
 * 补全缺失的 ID、类型等默认值，确保每个节点结构完整。
 * @param skill 技能草稿
 * @returns 标准化的步骤数组
 */
...
/**
 * 构建以源节点 ID 为 key 的出边映射表。
 * 用于快速查找某个节点有哪些后续边。
 * @param skill 技能草稿
 * @returns { [sourceNodeId]: [edge, ...] }
 */
...
/**
 * 归一化技能节点列表：过滤非法项并补全 node_id。
 * @param skill 技能草稿
 * @returns 带有合法 node_id 的节点数组
 */
...
/**
 * 归一化技能边列表：过滤非法项并补全 source/target/condition/priority。
 * @param skill 技能草稿
 * @returns 带有合法字段的边数组
 */
...
/**
 * 在边数组中查找指定源节点的第 localIndex 条出边的全局索引。
 * @param edges 全部边数组
 * @param sourceId 源节点 ID
 * @param localIndex 该源节点下的第几条出边（从 0 开始）
 * @returns 在 edges 数组中的全局索引，未找到返回 -1
 */
...
/**
 * 生成一个在当前节点列表中不重复的节点 ID。
 * 优先使用 preferred 名称（清洗后），冲突时追加数字后缀。
 * @param nodes 现有节点列表
 * @param preferred 期望的节点 ID
 * @returns 唯一的节点 ID
 */
...
/**
 * 构建技能流程图的画布布局：计算所有节点和边的坐标与 SVG 路径。
 * 这是流程图渲染的核心算法，负责：
 * 1. 拓扑排序确定节点的层级（rank）
 * 2. 计算每个节点的 x/y 坐标
 * 3. 为每条边生成避碰的 SVG 路径
 * @param skill 技能草稿
 * @returns 包含 nodes、edges、尺寸的画布布局对象
 */
 ...
 *
 * 算法步骤：
 * 1. 从起始节点出发做 BFS，标记所有可达节点
 * 2. 使用迭代松弛法计算每个节点的 rank（最小层级）
 * 3. 按 rank 分层，同层内按原始索引排序
 * 4. 计算每层的 y 坐标和节点的 x 坐标
 *
 * @param skill 技能草稿（包含 edges 用于连通性分析）
 * @param nodes 归一化后的节点列表
 * @returns 分层布局结果（含 layers 和坐标）
 */
...
/**
 * 计算侧边前向绕行边的车道 X 坐标（避碰）。
 * 当前向边需要绕过中间节点时，在源/目标节点的左侧或右侧寻找一条
 * 不与其他节点碰撞的垂直车道。
 * @param source 源节点
 * @param target 目标节点
 * @param nodes 全部画布节点（用于碰撞检测）
 * @param canvasWidth 画布宽度
 * @param siblingIndex 同源出边的序号（影响车道偏移）
 * @param incomingIndex 同目标入边的序号（影响车道偏移）
 * @returns 车道的 X 坐标
 */
