/**
 * @file pages/OpenPlatformPage.tsx
 * @description 开放广场页面 — 数字员工与资源的公共展示与复制入口。
 *
 * 该页面是企业端"开放平台"的核心，以多列卡片布局展示：
 * - 数字员工广场：已发布的员工卡片，可直接发起对话
 * - 知识库广场：公共知识库，可复制到当前员工
 * - 技能广场：浏览器/MCP 等通用技能，可复制
 * - SOP 广场：已发布的 SOP 流程
 * - 工具广场：HTTP 工具等，可复制
 *
 * 支持点击卡片查看详情抽屉、复制资源到当前员工、删除自己创建的资源。
 * 通过 URL 参数 kind 控制默认展开的列。
 */
...
export default function OpenPlatformPage({
  ...
  /**
   * 生成资源项的唯一删除标识键。
   * @param platformKind - 资源类型
   * @param item - 资源项
   * @returns "类型:标识" 格式的删除键
   */
...
/**
 * 判断知识库是否为"空的默认知识库"（名称为"默认知识库"且无任何文档/桶/片段）。
 * 用于在广场列表中过滤掉无意义的空默认知识库。
 * @param item 知识库数据
 * @returns 是空默认知识库返回 true
 */
