/**
 * @file pages/TutorialPage.tsx
 * @description StaffDeck 产品教程/文档页面。
 *
 * 以单页文档形式展示 StaffDeck 数字员工运营平台的完整介绍：
 * - 项目简介、安装说明、快速开始
 * - 八大核心功能（数字员工、知识库、技能、SOP、工具、记忆、定时任务、追踪反馈）
 * - 运行闭环、架构概览、执行流程
 * - 配置参考、开发指南、推荐场景和常见问题
 *
 * 页面左侧为固定目录导航（锚点跳转），右侧为正文内容。
 * 支持 URL hash 自动滚动到对应章节。
 */
...
/**
 * 教程文档页面主组件。
 * 渲染 Hero 区、目录导航和全部正文章节。
 * 挂载时读取 URL hash 并平滑滚动到对应章节。
 * @returns 渲染好的教程文档页面
 */
    ...
    <main className="tutorial-doc-page">
      <section className="tutorial-doc-hero" id="intro">
        <div className="tutorial-doc-hero-copy">
          <span className="ui-typography tutorial-doc-eyebrow">广通服数字员工系统 Docs</span>
          <h1 className="ui-typography">企业数字员工运行时，从配置到持续运营</h1>
          <p className="ui-typography">
            广通服数字员工系统——把模型、数字员工、知识库、技能、SOP、工具、记忆、定时任务和 Trace 放到一条链路里，
      ...
      <section className="tutorial-doc-section tutorial-doc-intro-panel">
        <div>
          ...
          <p className="ui-typography">
            广通服数字员工系统的核心不是“能聊天”，而是让一个真实岗位拥有自己的配置、资源、执行规则和运营记录。
      ...
      <section className="tutorial-doc-section" id="core-features">
        <SectionHeading
          ...
          body="PilotDeck 把能力分成 WorkSpace、Router、Memory、Always On、Gateway；广通服数字员工系统对应到企业数字员工场景，重点是岗位边界、知识治理、流程执行、工具调用和运营复盘。"
        />
...
/**
 * 文档章节标题组件：小标签（eyebrow）+ 主标题 + 描述正文。
 * @param props.eyebrow 章节分类小标签
 * @param props.title 章节主标题
 * @param props.body 章节描述正文
 * @returns 渲染好的章节标题区块
 */
