 ...
 * @description 技能（SOP）管理页面。
 *
 * 管理数字员工的技能/SOP 流程，核心功能包括：
 * - 技能列表（表格 + 移动端卡片，支持搜索筛选、状态筛选、前端分页）
 * - 技能排行榜（按调用次数、正/负反馈排序，支持当前员工/全局两种维度）
 * - 新建/编辑/复制/删除技能
 * - 启用/停用/发布技能版本
 * - 从广场导入技能
 * - 查看版本历史和分支同步状态
 *
 * 数据通过 /api/enterprise/skills 系列接口交互，
 * 排行榜数据通过 /api/enterprise/skills/ranking 接口获取。
 */
...
export default function SkillsPage({
  ...
  /**
   * 确认删除技能：在分支模式下为"移除"，在全局模式下为"删除"。
   */
  async function confirmDelete() {
    const row = deleteTarget;
    if (!row) return;
    // 分支模式（选中了具体员工）下仅移除该员工的技能引用
